...
function generateQuestions() {
    ...
    for (let i = 0; i < 10; i++) {
        // Kies random antwoord-eenheid: m/s of km/h
        const answerUnit = random() > 0.5 ? 'm/s' : 'km/h';
        
        let distance, time, speed;
        let distanceUnit, timeUnit;
        
        if (answerUnit === 'm/s') {
            // Antwoord in m/s
            // Kies random afstand eenheid (m of km)
            const useMeters = random() > 0.5;
            
            if (useMeters) {
                // Afstand in meters
                distanceUnit = 'm';
                distance = floor(random(100, 5000)); // 100-5000 meter
                
                // Tijd in seconden of minuten
                const useSeconds = random() > 0.5;
                if (useSeconds) {
                    timeUnit = 'sec';
                    time = floor(random(10, 300)); // 10-300 seconden
                } else {
                    timeUnit = 'min';
                    time = floor(random(1, 20)); // 1-20 minuten
                    // Converteer naar seconden voor berekening
                ...
                distanceUnit = 'km';
                distance = (floor(random(1, 50)) * 10) / 10; // 0.1-5 km met 1 decimaal
                
                // Tijd in minuten of uren
                const useMinutes = random() > 0.5;
                if (useMinutes) {
                    timeUnit = 'min';
                    time = floor(random(5, 60)); // 5-60 minuten
                    // Converteer km en min naar m/s
                    speed = (distance * 1000) / (time * 60);
                } else {
                    timeUnit = 'uur';
                    time = (floor(random(1, 10)) * 10) / 10; // 0.1-1 uur met 1 decimaal
                    // Converteer km en uur naar m/s
            ...
            // Antwoord in km/h
            // Kies random afstand eenheid (m of km)
            const useKm = random() > 0.5;
            
            if (useKm) {
                // Afstand in kilometers
                distanceUnit = 'km';
                distance = floor(random(5, 200)); // 5-200 km
                
                // Tijd in minuten of uren
                const useHours = random() > 0.5;
                if (useHours) {
                    timeUnit = 'uur';
                    time = (floor(random(1, 20)) * 10) / 10; // 0.1-2 uur met 1 decimaal
                    speed = distance / time;
                } else {
                    timeUnit = 'min';
                    time = floor(random(10, 120)); // 10-120 minuten
                    // Converteer minuten naar uren
                    speed = distance / (time / 60);
                }
                ...
                // Afstand in meters
                distanceUnit = 'm';
                distance = floor(random(500, 10000)); // 500-10000 meter
                ...
                if (useMinutes) {
                    timeUnit = 'min';
                    time = floor(random(5, 60)); // 5-60 minuten
                    // Converteer m en min naar km/h
                    speed = (distance / 1000) / (time / 60);
                } else {
                    timeUnit = 'sec';
                    time = floor(random(30, 300)); // 30-300 seconden
                    // Converteer m en s naar km/h
                    speed = (distance / 1000) / (time / 3600);
                }
            }
        }
        
        // Rond snelheid af op 2 decimalen
        speed = Math.round(speed * 100) / 100;
        
        // Format met komma
        let distanceStr = distance.toString().replace('.', ',');
        let timeStr = time.toString().replace('.', ',');
        let speedStr = speed.toString().replace('.', ',');
        
        let text = `afstand: ${distanceStr} ${distanceUnit}\ntijd: ${timeStr} ${timeUnit}\nsnelheid\n in ${answerUnit} = ?`;
        let answer = speedStr;
...
function drawBlock(block) {
    ...
    // Check of het een breuk is (bevat /)
    if (block.text.includes('/') && !block.text.includes('m/s') && !block.text.includes('km/h')) {
        // Splits breuk in teller en noemer (maar niet voor m/s of km/h)
        let parts = block.text.split('/');
        let numerator = parts[0].trim();
        let denominator = parts[1].trim();
        
        // Tekstgrootte voor breuken
        textSize(18);
        ...
        // Teken teller (boven)
        text(numerator, centerX, centerY - 12);
        ...
        // Teken noemer (onder)
        text(denominator, centerX, centerY + 12);
        
    } else if (block.text.includes('\n')) {
        // Multi-line tekst (voor vragen)
